  ...
  if ("geolocation" in navigator) {
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        // 1️⃣ Get country code using OpenStreetMap Nominatim
        ...
        if (stocks.length === 0) {
          ...
          <ul>
            ${stocks
              .map((stock) => `<li>${stock.name} (${stock.symbol})</li>`)
              .join("")}
          </ul>
        `;
        }
