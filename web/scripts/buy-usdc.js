document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateRateDisplay() {
    ...
    if (rateText) {
      rateText.textContent = `Exchange Rate: 1 USDC ≈ ${liveRate.toFixed(
        4
      )} EUR`;
    }
  }
...
async function fetchCryptoPrices() {
  try {
    const res = await fetch(
      "https://api.coingecko.com/api/v3/simple/price?ids=bitcoin,ethereum,tether,dogecoin&vs_currencies=usd&include_24hr_change=true"
    );
    const data = await res.json();
